...
VideoPlayer::VideoPlayer(QQuickItem* parent) : QQuickFramebufferObject(parent), mpv_gl(nullptr) {
    // qDebug() << "VideoPlayer is initializing!";
    mpv = Player::instance()->engine()->getMpvHandle();
    connect(this, &VideoPlayer::onUpdate, this, &VideoPlayer::doUpdate, Qt::QueuedConnection);
}
