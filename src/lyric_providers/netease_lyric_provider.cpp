...
bool NeteaseLyricProvider::accepted(const Media &media) {
    ...
    if (cm.startsWith("163 key(Don't modify):"))
        return true;
    return false;
}
...
void NeteaseLyricProvider::getLyricsRequest(const Media &media) {
    ...
    auto url = BitWaveConstants::getNeteaseLyricsApiAddress(QString("%1").arg(musicId));
    QNetworkRequest req;
    req.setUrl(url);
    req.setHeader(QNetworkRequest::UserAgentHeader, BitWaveConstants::cliApiUa());
...
void NeteaseLyricProvider::onRequestFinished(QNetworkReply *reply) {
    ...
    if (jsonObj.value("nolyric").toBool()) {
        emit this->lyricsIsReady(true, tr("[00:00.00]Pure Music"), tr("[00:00.00]Enjoy."));
        return;
    }
    if (jsonObj.value("uncollected").toBool()) {
        emit this->lyricsIsReady(true, tr("[00:00.00]Lyrics not found"), QString());
        return;
    }
    auto rawLyrics = jsonObj.value("lrc").toObject().value("lyric").toString();
    auto trLyrics = jsonObj.value("tlyric").toObject().value("lyric").toString();
